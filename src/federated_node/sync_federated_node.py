...
class SyncFederatedNode:
    ...
    def update_parameters(self, local_parameters: Parameters, num_examples: int = None):
        ...
        while len(parameters_from_other_nodes) < self.num_nodes - 1:
            ...
            LOGGER.info(f"Got {len(parameters_from_other_nodes)} parameters from other nodes.")
            LOGGER.info(f"Waiting for {self.num_nodes - 1 - len(parameters_from_other_nodes)} more.")
